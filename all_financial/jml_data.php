<?php
$q_jml_where = " CHANNELCODE = '$channelcode'";
...
foreach ($tran_type as $key => $value) {
    $jml_tran_cat = mysqli_fetch_array(mysqli_query($con, "SELECT count(id) as jml_tran_cat FROM $tabel WHERE $q_jml_where AND TransactionCategory = '".$value."'"));
    ?>
